let wormPosition = {top:200,left:200}
...
function updateWormBody() {
    if (wormBody.length > score) {
        let wormTail = wormBody.shift();  // Remove the oldest part of the worm
        document.getElementById(wormTail.id).remove();  // Remove the old segment from the DOM
    }
    ...
    // Push the new segment to the wormBody array
    wormBody.push({
    ...
    // Append the new segment to the game board
